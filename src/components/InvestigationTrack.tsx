...
interface InvestigationTrackProps {
  initialState?: GameState;
  onTokenMove?: (tokenType: TokenType, tokenId: number | null, newPosition: number) => void;
}

const InvestigationTrack: React.FC<InvestigationTrackProps> = ({ initialState, onTokenMove }) => {
  ...
  useEffect(() => {
    ...
    // ctx.fillText('調査トラック', canvas.width / 2, 25);
  ...
  return (
    <div className="flex flex-col items-center p-4">
      ...
      {/* カード表示 */}
      <div className="w-40 h-56 bg-white rounded-lg shadow-md border-2 border-gray-300 overflow-hidden my-5 flex flex-col">
        <div className="bg-gray-100 p-2 border-b border-gray-300 text-center font-bold text-sm">
          記者の執念
        </div>
        <div className="flex items-center justify-center p-3 bg-gray-50">
          <span className="text-2xl font-bold text-gray-700">3</span>
        </div>
        <div className="p-2 text-xs flex-grow overflow-y-auto">
          イニシアチブマーカーを2マス自分側に移動
        </div>
        <div className="flex border-t border-gray-300">
          <button className="flex-1 bg-blue-500 text-white py-2 border-none text-sm font-semibold">
            アクション
          </button>
          <div className="w-px bg-white"></div>
          <button className="flex-1 bg-green-500 text-white py-2 border-none text-sm font-semibold">
            数値
          </button>
        </div>
      </div>
